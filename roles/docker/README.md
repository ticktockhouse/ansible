...
An [Ansible](http://www.ansible.com/home) role to:
...
Heavily borrowed from https://github.com/nickjj/ansible-docker, because why re-invent the wheel..?
...
other options with the `docker_daemon_json` variable which is explained later.
...
- `config_path` defaults to your `docker_login_become_user`'s `$HOME` directory
...
- `user` defaults to the first `docker_users` user or root if that's not available
